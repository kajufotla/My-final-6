...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const captureCurrentFormSnapshot = () => ({
      ...
      custAddress: cache.custAddress?.value || '', receiptNumber: cache.receiptNumber?.value || '', poNumber: cache.poNumber?.value || '', refNumber: cache.refNumber?.value || '',
      issueDate: cache.issueDate?.value || '', dueDate: cache.dueDate?.value || '',
  ...
  /* INSTANT VALIDATION BUG FIX: Remove required error immediately upon typing valid input */
  document.addEventListener('input', (e) => {
    if(e.target.classList.contains('req-field') || e.target.classList.contains('error')) {
      runSmartFieldValidation(e.target, e.target.type || 'string', cache);
    }
    if(['INPUT', 'TEXTAREA', 'SELECT'].includes(e.target.tagName)) { updatePreview(); autoSaveDraftAction(); }
  });

  // TABS ISSUE FIX - Fully init tabs without overwriting
  ...
  // ENTERPRISE UPGRADE: 4-BUTTON ARCHITECTURE (HISTORY, PDF, PRINT, RESET)

  // 1. Reset Feature (Only Clear Data, Keep Layout)
  document.getElementById('btnReset')?.addEventListener('click', () => {
    if(confirm("Reset user-entered data? The layout configuration, logo, and company profile will be preserved.")) {
      ['custName', 'custCompany', 'custEmail', 'custPhone', 'custAddress', 'discountVal', 'taxRate', 'shippingCost', 'notes', 'terms', 'payUrl', 'bankDetails', 'poNumber', 'refNumber', 'bankAccTitle', 'bankName', 'bankAccNo', 'bankIban', 'bankSwift', 'bankBranch', 'bankCode', 'bankRef'].forEach(id => {
          if(cache[id]) cache[id].value = '';
      });
  ...
  document.getElementById('btnSaveHistory')?.addEventListener('click', () => {
    ...
    const snap = captureCurrentFormSnapshot();
    snap.totalVal = cache.prevTotal?.textContent || '';
    snap.number = cache.receiptNumber?.value || '';
    snap.date = cache.issueDate?.value || '';
    snap.status = cache.invoiceStatus ? cache.invoiceStatus.value : 'Draft';
    
    historyLogs.push(snap);
  ...
  // 4. Print strictly using browser Print
  ...
  // FIX TAB LOAD BUG: Force initial active tab immediately upon load without overwriting HTML
  document.querySelector('.sticker-btn.active')?.click();
